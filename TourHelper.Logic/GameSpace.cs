...
namespace TourHelper.Logic
{
    ...
    {
        public delegate void RebaseListener(Coordinates c);
        ...
        private void InstantiatePrefab(string prefab, IEnumerable<Coordinates> coordinates)
        {
            ...
            if (Prefabs.ContainsKey(prefab)&& prefabType!=null)
            {
                ...
                foreach (Coordinates c in coordinates)
                {
                    ...
                    Vector3 position = _translator.GetCoordinates(c);
                    GameObject element = (GameObject)GameObject.Instantiate(prefabType);
